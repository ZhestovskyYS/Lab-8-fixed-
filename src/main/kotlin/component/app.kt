...
class App : RComponent<AppProps, AppState>() {
    ...
    override fun RBuilder.render() {
        ...
        switch {
            route("/lessons",
                exact = true,
                render = {
                    anyList(state.lessons, "Lessons", "/lessons", deleteLesson())
                }
            )
            route("/students",
                exact = true,
                render = {
                    anyList(state.students, "Students", "/students", deleteLesson())
                }
            )
    ...
    fun addStudent(): (Event) -> Unit = {
        ...
        setState{
            students += Student(firstName.value, sureName.value)
            presents += arrayOf(Array(state.students.size){false})
        }
    }

    fun deleteStudent(): (Int) -> Unit ={index ->
        var redactedPresents = state.presents.toMutableList().apply{ removeAt(index)}.toTypedArray()
        val redactedStudents = state.students.toMutableList().apply{ removeAt(index)}.toTypedArray()
        setState{
    ...
    fun deleteLesson(): (Int) -> Unit = {index->
